...
t_game *ft_init_game(char *map_path)
{
	...
	game->mlx = mlx_init();	 // inicializar mlx
	if (!game->mlx)
		ft_end(5, NULL);					 // erro ao inicializar mlx
...
void init_player(t_game *game)
{
	...
	while (game->scene->map[++y])
	{
		...
		{
			if (game->scene->map[y][x] == 'N' || game->scene->map[y][x] == 'S' || game->scene->map[y][x] == 'E' || game->scene->map[y][x] == 'W')
			{
				game->player.x = 2;
				game->player.y = 5;
			}
